                ...
                <div class="card card-custom rdp_statistic_mg">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row mb-5">
                            <div class="col-md-8">
                                <Link
                                    :href="route('channel-types.create')"
                                    as="button"
                                    method="get"
                        ...
                        <v-server-table
                                ...
                                :columns="columns"
                                :options="options"
                                ref="channel-types-table"
                            >
                                <template v-slot:actions="{row}">
                                    <Link :href="route('channel-types.edit', {id: row.id})" method="get">
                                    ...
                                    <a href="javascript:;" @click="deleteChannelType(row)">
...
export default {
    ...
    data() {
        return {
            url: '/admin/channel-types-all-paginate',
            columns: [
                'id',
                'name',
                'description',
    ...
    methods: {
        deleteChannelType(row){
            Swal.fire({
                title: 'Удалить тип канала?',
                text: "Выбранный тип канала будет удален",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    axios.delete('/admin/channel-types/' + row.id).then(response => {
                        if (response.data.status === true) {
                            this.$notify({
                                group: 'foo',
                                type: 'success',
                                title: 'Удаление типа канала',
                                text: 'Тип канала успешно удален.',
                            });
                            Swal.fire(
                                'Удалено!',
                                'Тип канала удален',
                            ...
                            this.$refs['channel-types-table'].refresh();
                        }
