                ...
                <div class="card card-custom rdp_statistic_mg">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row mb-5">
                            <div class="col-md-8">
                                <Link
                                    :href="route('backup-priorities.create')"
                                    as="button"
                                    method="get"
                        ...
                        <v-server-table
                                ...
                                :columns="columns"
                                :options="options"
                                ref="backups-priorities-table"
                            >
                                <template v-slot:actions="{row}">
                                    <Link :href="route('backup-priorities.edit', {id: row.id})" method="get">
...
export default {
    ...
    data() {
        return {
            url: '/admin/backup-priorities-all-paginate?',
            columns: [
                'id',
                'name',
                'created_at',
    ...
    methods: {
        deleteBackupObject(row){
            Swal.fire({
                title: 'Удалить приоритет бэкапа?',
                text: "Выбранный приоритет бэкапа будет удален",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    axios.delete('/admin/backup-priorities/' + row.id).then(response => {
                        if (response.data.status === true) {
                            this.$notify({
                                group: 'foo',
                                type: 'success',
                                title: 'Удаление приоритета бэкапа',
                                text: 'Приоритет бэкапа успешно удален.'
                            });
                            Swal.fire(
                                'Удалено!',
                                'Приоритет бэкапа удален',
                                'success'
                            )
                            this.$refs['backups-priorities-table'].refresh();
                        }
