                ...
                <div class="card card-custom rdp_statistic_mg">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row mb-5">
                            <div class="col-md-8">
                                <Link
                                    :href="route('internet-speed.create')"
                                    as="button"
                                    method="get"
                        ...
                        <v-server-table
                                ...
                                :columns="columns"
                                :options="options"
                                ref="internet-speed-table"
                            >
                                <template v-slot:actions="{row}">
                                    <Link :href="route('internet-speed.edit', {id: row.id})" method="get">
                                    ...
                                    <a href="javascript:;" @click="deleteInternetSpeed(row)">
...
export default {
    ...
    data() {
        return {
            url: '/admin/internet-speed-all-paginate',
            columns: [
                'id',
                'name',
                'description',
    ...
    methods: {
        deleteInternetSpeed(row){
            Swal.fire({
                title: 'Удалить скорость интернета?',
                text: "Выбранная скорость интернета будет удалена",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    axios.delete('/admin/internet-speed/' + row.id).then(response => {
                        if (response.data.status === true) {
                            this.$notify({
                                group: 'foo',
                                type: 'success',
                                title: 'Удаление скорости интернета',
                                text: 'Скорость интернета успешно удалена.',
                            });
                            Swal.fire(
                                'Удалено!',
                                'Скорость интернета удалена',
                                'success'
                            )
                            this.$refs['internet-speed-table'].refresh();
                        }
