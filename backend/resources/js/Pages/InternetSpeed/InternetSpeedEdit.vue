...
export default {
    ...
    methods: {
        updateChannelType() {
            ...
            axios.put('/admin/internet-speed/' + this.id, this.form)
                .then(res => {
                    if (res.status === 200){
                        this.$notify({
                            title: "Обновление скорости интернета",
                            text: "Скорость интернета обновлена!",
                            speed: 1000,
                            duration: 1000,
                            type: 'success'
                        });
        ...
        getBackupDay(){
            axios
                .get(`/admin/internet-speed/${this.id}`, this.form)
                .then(res => {
                    let data = res.data.data.internetSpeed
