...
export default {
    ...
    methods: {
        updateBackupObject() {
            ...
            axios.put('/admin/backup-days/' + this.id, this.form)
                .then(res => {
                    if (res.status === 200){
                        this.$notify({
                            title: "Обновление типа дня бэкапа",
                            text: "Типа дня бэкапа обновлен!",
                            speed: 1000,
                            duration: 1000,
                            type: 'success'
                        });
        ...
        getBackupDay(){
            axios
                .get(`/admin/backup-days/${this.id}`, this.form)
                .then(res => {
                    let data = res.data.data.backupDay
                    this.form = {
                        name: data.name,
